...
describe('ProductsHeader — Sort Dropdown', () => {
  ...
  beforeEach(() => {
    const result = setupShallowMount(ProductsHeader)
  ...
  describe('initial state', () => {
    it('renders the section title', () => {
      expect(get('products-title').text()).toBe('Explore Our Products')
    })

    it('renders the sort label', () => {
      expect(get('sort-label').text()).toBe('Sort by')
    })

    it('displays "Highest Rating" as the default selected option', () => {
      expect(get('dropdown-selected').text().trim()).toBe('Highest Rating')
    })

    it('does not render the dropdown menu on mount', () => {
      expect(get('dropdown-menu').exists()).toBe(false)
    })
  })

  describe('toggle behaviour', () => {
    it('opens the dropdown menu when the trigger is clicked', async () => {
      await get('dropdown').trigger('click')
      expect(get('dropdown-menu').exists()).toBe(true)
    })
  ...
  describe('menu options', () => {
    ...
    it('renders "Highest Rating" option', () => {
      expect(get('dropdown-item-highest-rating').exists()).toBe(true)
    })

    it('renders "Price: Low to High" option', () => {
      expect(get('dropdown-item-price--low-to-high').exists()).toBe(true)
    })

    it('renders "Price: High to Low" option', () => {
      expect(get('dropdown-item-price--high-to-low').exists()).toBe(true)
    })
  })
  ...
  describe('option selection', () => {
    ...
    it('updates the selected label when "Price: Low to High" is chosen', async () => {
      await get('dropdown-item-price--low-to-high').trigger('click')
      expect(get('dropdown-selected').text().trim()).toBe('Price: Low to High')
    })

    it('updates the selected label when "Price: High to Low" is chosen', async () => {
      await get('dropdown-item-price--high-to-low').trigger('click')
      expect(get('dropdown-selected').text().trim()).toBe('Price: High to Low')
    })

    it('closes the menu after an option is selected', async () => {
      await get('dropdown-item-highest-rating').trigger('click')
      expect(get('dropdown-menu').exists()).toBe(false)
    })
  ...
  describe('click outside to close', () => {
    ...
    it('keeps the dropdown open when clicking inside it', async () => {
      ...
      const dropdownEl = get('dropdown').element
      dropdownEl.dispatchEvent(new MouseEvent('click', { bubbles: true }))
      await wrapper.vm.$nextTick()

      expect(get('dropdown-menu').exists()).toBeDefined()
    })
  })
